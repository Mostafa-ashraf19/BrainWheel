...
def general_test():
	for cache in CV.loop(l_img=True, r_img=True, depth_map=True, depth_map_img=True, point_cloud=True):
		...
		# min_dist, is_close = CV.is_close_to_collision(pred_bbox = od_bbox, return_min_dist=True)
		# is_close = "CLOSE!!!" if is_close else "not close"
		# print(min_dist, '\t=>', is_close)

		min_dist, is_close = CV.is_close_to_collision_simple(depth_map=depth_map, return_min_dist=True)
		is_close = "CLOSE!!!" if is_close else "not close"
		print(min_dist, '\t=>', is_close)
