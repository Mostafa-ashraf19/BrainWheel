...
#     demo.record_data()
#
#     # print(len(demo.data),'time taken :',time.time()-start)
#     demo.save_data('youm laziz')
#     # print('End')
#
#
# # from multiprocessing import Process
# # p = Process(target=read_data, args=())
# # p.start()
# # p.join()
# read_data()
# # print('start 5s')
...
# demo.save_data('youm laziz2')
    ...
    def Extract_features(self,trails):
        ...
        for trail in trails:
            feature = []
            for feature_num in range(14):#[5,6,7,8,9,13]: #[2, 3, 4, 6, 7, 8, 9, 11] 5?
    ...
    def signals_test(self, users_list=[], matrices=[], PCA=False, COMMON_AVG_Noise=False):
        ...
        y = trails[0+4][6]
        # print(y)
        y = butter_lowpass_filter(y, 40, 256)
        y = butter_highpass_filter(y, 5, 256)
    ...
    def get_data(self,users_list=[]):
        ...
        # print(type(frame_with_data))
    def encod_split(self,X,labels):
        lab_enc = preprocessing.LabelEncoder()
        encoded = lab_enc.fit_transform(labels)
        return  train_test_split(X, encoded, random_state=42, train_size=.8)
    ...
    def train_data(self, users_list=[], matrices=[], PCA=True, COMMON_AVG_Noise=True):
        ...
        self.PCA = PCA
        self.COMMON_AVG_Noise = COMMON_AVG_Noise
        trails , labels = self.get_data(users_list=users_list)
        ...
        x_train, x_test, y_train, y_test = self.encod_split(X, labels)
        #apply pca
        if PCA==True:
...
# demo.train_data(users_list= ['Girgis_2.1'], PCA=True, COMMON_AVG_Noise=True)
# demo.signals_test(users_list=['Girgis_1.7O'],PCA=True,COMMON_AVG_Noise=True)