...
class BCI:
    ...
    def Extract_features(self,trails):
        ...
        for trail in trails:
            feature = []
            for feature_num in [2, 3, 4, 6, 7, 8, 9, 11]:
    ...
    def train_data(self, users_list=[], matrices=[], PCA=True, COMMON_AVG_Noise=True):
        ...
        x_train, x_test, y_train, y_test = train_test_split(X, encoded, random_state=0, train_size=.8)
        ...
        self.model = Pipeline([
            ...
            ("svm_clf", SVC(kernel="poly", degree=1, coef0=1, gamma=.8, C=7))
        ])
...
>>>>>>> ea3b0d5d5c6be86e23ef3f9ee4f07bfeb3f3a42e
